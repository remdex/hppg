...
$paymentDetails = $resp_details->getPaymentDetails();
...
<form action="<?=erLhcoreClassDesign::baseurl('/shop/review/paypal_handler')?>/(token)/<?=$token?>/(paymentAmount)/<?=$paymentAmount?>/(currencyCodeType)/<?=$currencyCodeType?>/(paymentType)/<?=$paymentType?>/(payerID)/<?=$payer_id?>" method="post" name="ExpressCheckoutForm">
<table width=400>
<tr>
