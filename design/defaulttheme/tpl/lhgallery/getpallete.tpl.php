...
<?php if ($mode == 'color') : ?>
    <a href="<?=erLhcoreClassDesign::baseurl('gallery/color')?>/(color)/<?=implode('/',$moreColors).$no_color.$resolution.$match?>"></a>
<?php elseif ($mode == 'search') : ?>
    <a href="<?=erLhcoreClassDesign::baseurl('gallery/search')?>/(keyword)/<?=urlencode($keyword)?>/(color)/<?=implode('/',$moreColors).$no_color.$resolution.$match?>"></a>    
<?php endif;?>
</div>
<?endforeach;?>
</div>