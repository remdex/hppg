...
foreach ($items as $item) : ?>
    <div class="album-thumb<?=!($counter % 4) ? ' left-thumb' : ''?>">
       ...
       <div class="right">
               ...
               <input title="<?=erTranslationClassLhTranslation::getInstance()->getTranslation('gallery/album_list','Album priority')?>" type="text" class="inputfield smlinputfield" style="width:20px;" name="Position[]" value="<?=$item->pos?>" /><input type="hidden" name="AlbumIDs[]" value="<?=$item->aid?>" />
       </div>
       </div>
