...
<div class="nav-container">
    ...
    <?php if ($pages->current_page != 1) : ?>    
        <a class="previous" onclick="hw.goToPage('<?php echo $pages->serverURL,$pages->prev_page,$pages->querystring?>')"><?=erTranslationClassLhTranslation::getInstance()->getTranslation('core/paginator','Previous')?></a>    
    <?php endif;?>
    ...
    $needNoBolder = true;
    	...
    	<?php else : ?>  		
           <a class="no-b" title="<?=erTranslationClassLhTranslation::getInstance()->getTranslation('core/paginator','Go to page')?> <?=$i?> <?=erTranslationClassLhTranslation::getInstance()->getTranslation('core/paginator','of')?> <?=$pages->num_pages?>" onclick="hw.goToPage('<?php echo $pages->serverURL,$pageURL,$pages->querystring?>')"><?=$i?></a>
        <?php endif;        
        ?> ... <?php
        endif;
        ...
        $needNoBolder = false;
			    ...
			    <a class="<?=$noBolderClass?>" title="<?=erTranslationClassLhTranslation::getInstance()->getTranslation('core/paginator','Go to page')?> <?=$i?> <?=erTranslationClassLhTranslation::getInstance()->getTranslation('core/paginator','of')?> <?=$pages->num_pages?>" onclick="hw.goToPage('<?php echo $pages->serverURL,$pageURL,$pages->querystring?>')"><?=$i?></a>
   ...
   $pageURL = $i > 1 ? '/(page)/'.$i : '';
		...
		<?php else : ?>
		    <a class="paginate" onclick="hw.goToPage('<?php echo $pages->serverURL,$pageURL,$pages->querystring;?>')" ><?=$i?></a>							
    <?php endif; endfor; endif;

    if ($pages->current_page != $pages->num_pages): ?>    
    
    <a class="next" onclick="hw.goToPage('<?php echo $pages->serverURL,'/(page)/',$pages->next_page,$pages->querystring?>')"><?=erTranslationClassLhTranslation::getInstance()->getTranslation('core/paginator','Next')?></a>
