...
{			
  ...
  $profile->setEnvironment($paymentHandler->getSettingValue('sandboxmode')->value == 1 ? 'sandbox' : 'live'); 
   ...
   $url='http://'.$serverName.erLhcoreClassDesign::baseurl('shop/reviewcredits/paypal_handler');
   
   $currencyCodeType=$basket->currency;
      
   // paymentType is ActionCodeType in ASP SDK
   $paymentType=$paymentHandler->getSettingValue('payment_type')->value;
	      
   $returnURL = $url.'/?paymentAmount='.$basket->amount.'&currencyCodeType='.$currencyCodeType.'&paymentType='.$paymentType;
   $cancelURL = 'http://'.$serverName.erLhcoreClassDesign::baseurl('shop/cancelcredits/paypal_handler');
   ...
   $ec_details->setBuyerEmail($basket->user->email);
...
//   $ec_details->setcpp_header_image('http://img91.imageshack.us/img91/8738/paypalsdk.jpg');  
   ...
   $amt_type->setval($basket->amount, 'iso-8859-1');
   $ec_details->setOrderTotal($amt_type);
	...
	$itemTotal_type =& PayPal::getType('BasicAmountType');
   	...
   	$itemTotal_type->setval($basket->amount/*$itemamt*/, 'iso-8859-1');
...
$tpl = erLhcoreClassTemplate::getInstance( 'lhshop/handlers/paypal_handler/processcredit.tpl.php');
